...
class TestbenchEnv(gym.Env):
    ...
    def __init__(self, server, client, size = 'Small'):
        self.size = size
        self.server = server
        self.client = client
    ...
    # Set up server side port connection to transmit actions to the R simulation
        ...
        # self.n_queues = float(self.n_queues)
        # print(type(self.n_queues))
    ...
    def _receive_client_data(self, conv_float = False):
    ...
    def reset(self, seed=None, options=None):
        ...
        observation = self._receive_client_data()
        signal = "intial_observation"
        self._send_client_data(signal)
        return_signal = self._receive_client_data()
        return observation
    ...
    def step(self, action):
        ...
        terminated = False
        if 'terminate' in observation.split():
            terminated = True
            reward = None
            observation = None
        else:
            observation = np.array([float(i) for i in observation.split(",")])
            self._send_client_data('reward signal')
            reward = self._receive_client_data()
            reward = float(reward)
